...
import { allowUserToSharedCollection, auth, findUserInAllowUsers } from '../../firebase';
...
export default function Header() {
  ...
  const sharedNoteRecord = useFindEnableUserToShareNotes(currentUser?.uid || '0')
  ...
  const handleEnableShareNotes = async () => {
    ...
    try {
      ...
      if(data.empty) {
        const response = await CreateRecordEnabledSharedNotes(currentUser.uid, currentUser.email)
        response.success ? updateToasterToSuccess(toastId, "Now any one share with you notes") : updateToasterToError(toastId, response.error)
      }
      else {
        let record = null;
        ...
        const msg = !record.is_allow ? "Now any one share with you notes" : "Notes Sharing blocked";
    ...
    <div style={{ marginBottom: '5rem' }}>
      <nav className="navbar navbar-expand-lg navbar-light bg-light shadow fixed-top p-1">
        <div className="container">
          <Link to="/" className="navbar-brand">R-K NOTES</Link>
          ...
          <div className="collapse navbar-collapse" id="navbarResponsive">
            <ul className="navbar-nav ms-auto">
              <li className="nav-item m-auto">
                <Link to="/my-notes" className="nav-link p-2">My Notes</Link>
              </li>
              <li className="nav-item m-auto">
                <Link to="/assign-notes" className="nav-link p-2">Assign Notes</Link>
              </li>
              <div className="dropdown m-auto mx-3 bold h6 p-2 text-center">
                <button className="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false">
                  WELCOME BACK | <span className='text-light bold'> {currentUser.displayName}</span>
                </button>
